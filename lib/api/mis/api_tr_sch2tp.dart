...
class TrSch2tp {
  ...
  @ApiMethod(path: 'accountId/{accountId}/start/{start}/end/{end}/settlement/{settlement}/summary')
  Future<ApiResponse> summaryForAccount(String accountId, String start, String end,
  ...
  @ApiMethod(path: 'accountId/{accountId}/subaccountId/{subaccountId}/start/{start}/end/{end}/settlement/{settlement}/summary')
  Future<ApiResponse> summaryForSubaccount(String accountId,
      String subaccountId, String start, String end, int settlement) async {
    var out = await _getSummary(accountId, subaccountId, start, end, settlement);
    return ApiResponse()..result = json.encode(out);
  }
    ...
    var pipeline = [
      ...
      },
      {
        '\$project': {
          ...
          'Energy Transaction Units Dollars': {'\$sum': '\$Energy Transaction Units Dollars'},
          'Volumetric Measures': {'\$sum': '\$Volumetric Measures'},
          'Volumetric Measures Dollars': {'\$sum': '\$Volumetric Measures Dollars'},
          'Total ISO Schedule 2 Charges': {'\$sum': '\$Total ISO Schedule 2 Charges'},
        }
      },
    ];
