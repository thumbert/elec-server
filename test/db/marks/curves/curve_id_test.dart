...
void tests() async {
  group('CurveIds API:', () {
    ...
    test('get all commodities', () async {
      var aux = await http.get('$shelfRootUrl/curve_ids/v1/commodities',
    ...
    test('get all regions for commodity electricity', () async {
      var aux = await http.get(
          '$shelfRootUrl/curve_ids/v1/commodity/electricity/regions',
          headers: {'Content-Type': 'application/json'});
    ...
    test('get energy curves for electricity, isone', () async {
      var aux = await http.get(
          '$shelfRootUrl/curve_ids/v1/data/commodity/electricity'
    ...
    test('get curve details for curveId isone_energy_4004_da_lmp', () async {
      var aux = await http.get(
          '$shelfRootUrl/curve_ids/v1/data/curveId/isone_energy_4004_da_lmp',
          headers: {'Content-Type': 'application/json'});
    ...
    test('get curve details for curveId isone_energy_4000_da_lmp', () async {
      var aux = await http.get(
          '$shelfRootUrl/curve_ids/v1/data/curveId/isone_energy_4000_da_lmp',
          headers: {'Content-Type': 'application/json'});
      var xs = json.decode(aux.body);
    ...
    test('get curve details for two curveIds', () async {
      ...
      expect(x.length, 2);
      var aux = await http.get(
          '$shelfRootUrl/curve_ids/v1/data/curveIds/${curves.join('|')}',
          headers: {'Content-Type': 'application/json'});
      var xs = json.decode(aux.body) as List;
      expect(xs.length, 2);
    });
    test('get mass hub daily volatility', () async {
      var aux = await http.get(
          '$shelfRootUrl/curve_ids/v1/data/curveId/isone_volatility_4000_da_daily',
          headers: {'Content-Type': 'application/json'});
      var xs = json.decode(aux.body);
