...
void downloadFile() async {
  ...
  var dir = (env['HOME'] as String) + '/Downloads/Archive/PnodeTable/Raw/';
...
void tests() async {
  group('Ptid table tests:', () {
    ...
    test('Get all the ptid information for one date (http)', () async {
      ...
      var res = await get(url);
      var data = json.decode(json.decode(res.body)['result'] as String);
    ...
    test('Get the list of available dates for one ptid (http)', () async {
      ...
      var data = json.decode(aux['result'] as String) as List;
      expect(data is List, true);
