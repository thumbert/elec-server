...
Future<void> tests(String rootUrl, String rustServer) async {
  ...
  group('IESO rt generation API tests:', () {
    ...
    test('Get all generators/variables', () async {
      var url =
          '$rootUrl/ieso/rt/generation/v1/names/variables/date/2023-09-01';
      var aux = await http.get(Uri.parse(url));
    ...
    test('Get generation for one fuel', () async {
      var res = await api.getGenerationForFuel(
          'hydro', 'output', '2019-05-01', '2019-05-03');
      expect(res.length, 3);
      var v0 = (res.first['output'] as List);
  ...
  group('IESO RT Generation client tests:', () {
    ...
    test('get hourly rt generation one name', () async {
      ...
      var aux =
          await client.hourlyRtGeneration('ABKENORA', term, variable: 'output');
      expect(aux.length, 10 * 24);
      expect(aux.first.interval,
          Hour.beginning(TZDateTime(Ieso.location, 2019, 5)));
    ...
    test('get hourly rt generation one name', () async {
      ...
      expect(aux['output']!.first.interval,
          Hour.beginning(TZDateTime(Ieso.location, 2019, 5)));
    ...
    test('get hourly rt generation by fuel', () async {
      var term = Term.parse('1May19-10May19', Ieso.location);
      var aux =
          await client.hourlyRtGenerationForType(IesoFuelType.nuclear, term);
      expect(aux.length, 10 * 24);
